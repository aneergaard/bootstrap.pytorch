"""
    @author: Alexander Neergaard Olesen
    mails: alexno@stanford.edu / aneol@dtu.dk / alexander.neergaard@gmail.com
    goal: extend DOSED algorithm with recurrent neural networks
...
class EventDetector(BaseDetector):
    ...
    def __init__(self, n_times=7680, n_channels=1, fs=256,
        ...
        self.recurrent = nn.Sequential(
            nn.GRU(
                input_size=4 * (2 ** k_max),
                hidden_size=4 * (2 ** k_max), num_layers=1,
        ...
        self.localization = nn.Sequential(
            nn.Conv2d(
                ...
                kernel_size=(2, n_times // (self.max_pooling ** self.k_max)))
...
#             x = x.transpose(2, 1)
        ...
        zdim = z.size()
        n_times = zdim[-1]
        z = z.permute(0, 3, 2, 1).reshape(batch, n_times, -1)
        z = self.recurrent(z)[0].view(batch, n_times, 2, -1).permute(0, 3, 2, 1)
